...
function kyrs_trans(){
    ...
    for(let i = 0;i<localStorage.getItem("sk");i++){
        for(let j = 0;j<=localStorage.getItem("sb");j++){
            localStorage.setItem(`T${i}${j}`,-1*document.getElementById(`${i}${j}`).value);
        }
    }
    for(let j = 0;j<=localStorage.getItem("sb");j++){
        if(j==localStorage.getItem("sb")) localStorage.setItem(`f${j}`, 0);
        else localStorage.setItem(`f${j}`,document.getElementById(`F${j}`).value);
    }
    localStorage.setItem('limit','min')
    for(let j = 0;j<localStorage.getItem("sb");j++){
        localStorage.setItem(`x${j}`,`x${j+1}`)
